...
use std::io::Read;
use std::path::Path;
use tiberius::{Client, Config, Query, AuthMethod, ColumnType, numeric::Numeric, time};
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let file_contents = std::fs::read_to_string("settings.json").expect("Should have been able to read the file");
    let settings: Settings = serde_json::from_str(&file_contents).expect("Should have been able to parse the file");
    ...
    results.first().unwrap().into_iter().enumerate().for_each(|(index_row,row)| {
        // println!("This is a row!: {:#?}", index_row);
        if index_row==0 {
            for (index_column,column) in row.columns().iter().enumerate() {
                worksheet.write( index_row as u32, index_column as u16 , column.name() );
            }
        }
        for (index_column,column) in row.columns().iter().enumerate() {
            ...
            match column.column_type() {
                ColumnType::Numericn => {
                    ...
                    if let Some(real_value) = value.unwrap() {
                        ...
                        worksheet.write( index_row as u32 + 1, index_column as u16 , real_value.value() as i32 );
                    }
                            ...
                            let real_value = match value {
                                Ok(num) => {
                                    if let Some(real_value) = value.unwrap() {
                                        ...
                                        worksheet.write( index_row as u32 + 1, index_column as u16 , real_value as i32 );
                                    }
                                },
                                Err(_) => continue,
                            };
                ...
                ColumnType::Datetime2 | ColumnType::Datetimen => {
                    ...
                    if let Some(real_value) = value.unwrap() {
                        ...
                        worksheet.write(index_row as u32 + 1, index_column as u16 , real_value.to_string() );
                    }
                    // println!("val:{:#?}",value.unwrap());
                },
                _ => {
